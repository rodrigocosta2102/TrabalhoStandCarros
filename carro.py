...
class Carro:
    ...
    def from_string(data_str):
        try:
            ...
            disponibilidade = disponibilidade.strip().lower() in ["true", "1", "sim"]
            return Carro(marca, modelo, cor, int(ano), float(preco), int(potencia), disponibilidade, matricula)
        except ValueError:
            print("Erro ao converter os dados do carro a partir da string fornecida.")
            return None
    ...
    def is_valid(self):
        ...
        if not self.matricula or not re.fullmatch(r"[A-Z]{2}-\d{2}-[A-Z]{2}", self.matricula):
            print("Erro: O campo 'matrícula' deve seguir o formato 'AA-00-AA'.")
            return False
